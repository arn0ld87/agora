...
2. Issue #106 trackt das seit dem Prod-Hardening-Slice: „Reverse-Proxy vor
   ...
   bauten ihre nginx-Konfig selbst nach der Skizze in `deployment-prod-like.md`.
3. Die Nginx-Skizze in `deployment-prod-like.md` (Zeile 148–175) enthielt keinen
...
| `docu/deployment-prod-like.md` | erweitert | Drei Topologie-Sections + Verifikations-Bullets |
...
| `docu/2026-05-03-slice-45-reverse-proxy-arbeitsprotokoll.md` | neu | dieses Dokument |
...
- **Sub-Slice 48 (F3 Gunicorn-Gevent):** `-k gevent` statt sync-Worker mit
  ...
  per Smoke verifizieren (siehe `docu/2026-04-29-prod-slice2-gunicorn.md`).
...
- **`frontend/dist` muss vom Operator gebaut werden** bevor der Sidecar startet.
  ...
  `docu/deployment-prod-like.md` (Sidecar-Nginx-Caveats).
