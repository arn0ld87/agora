...
**Quellen:** `SECURITY_REVIEW.md`, `docu/security-hardening.md`, `docu/SECURITY_REVIEW_SUMMARY.md`

## Status der Original-Findings aus `SECURITY_REVIEW.md`

| Finding | Severity | Status | Commit/Doc |
|---|---|---|---|
| F1 — API offen ohne Token | High | **DONE** | `5bd6b14`, `docu/2026-04-29-p0-1a-auth-fail-fast.md` |
...
`SECURITY_REVIEW.md` selbst ist überholt und sollte auf Snapshot-Status markiert werden.
...
### S1 — `SECURITY_REVIEW.md` Status-Sync

**Ziel:** Alte Review-Datei als historischen Snapshot markieren, Status-Tabelle vorne anhängen.

**Files:**
- Modify: `SECURITY_REVIEW.md` (Header + Status-Block)

- [ ] **Step 1: Header umbauen**

Oben in `SECURITY_REVIEW.md` einfügen:

```markdown
> **Stand 2026-04-29:** Dieser Review ist ein **historischer Snapshot** (2026-04-22).
> Aktueller Status der Findings siehe [`docu/2026-04-29-security-followup-plan.md`](./docu/2026-04-29-security-followup-plan.md)
...
```bash
git add SECURITY_REVIEW.md docu/2026-04-29-security-followup-plan.md
git commit -m "docs(security): mark SECURITY_REVIEW.md as historical snapshot"
```

**Arbeitsprotokoll:** `docu/2026-04-29-security-followup-s1-protokoll.md` (kurz, 5 Zeilen).
...
- Modify: [docu/security-hardening.md](docu/security-hardening.md) — Phase-2-Tabelle aktualisieren
...
In `CLAUDE.md` Sektion „Konfiguration": die Erwähnung `?token=<bearer>` entfernen. In `AGENTS.md` synchron. In `docu/security-hardening.md` Phase-2-Tabelle: Zeile zu `?token=` als „entfernt 2026-04-29" markieren.

- [ ] **Step 6: Commit**

```bash
git add backend/app/utils/auth.py backend/tests/test_auth.py CLAUDE.md AGENTS.md docu/security-hardening.md
git commit -m "feat(security): remove deprecated ?token= URL auth fallback"
```

**Arbeitsprotokoll:** `docu/2026-04-29-security-followup-s2-protokoll.md` mit Telemetrie-Evidence.
...
```bash
git add docker-compose.yml Dockerfile docu/security-hardening.md
git commit -m "feat(security): harden agora container with read-only rootfs and cap_drop"
```

**Arbeitsprotokoll:** `docu/2026-04-29-security-followup-s3-protokoll.md` mit Smoke-Log + finaler Cap-Liste.
...
- `SECURITY_REVIEW.md` zeigt prominent den aktuellen Status, niemand verwechselt das Dokument mit einer offenen TODO-Liste.
