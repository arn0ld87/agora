...
- 2026-05-07: Phase 1 Follow-up — `scripts/verify-deploy.sh` prueft Compose-Container ueber Docker-Running-State aus `docker compose ps -a` statt Health-Status und ersetzt den DOMPurify-Minifier-Grep durch Runtime-Bundle-Praesenz plus Source-Vertrag gegen `frontend/src/utils/markdown.ts`; Ziel ist ein stabiler `docker-image.yml::prod-proxy-smoke` auf `main`.
