...
async function tick(): Promise<void> {
  ...
  budget.value = res.data?.budget ?? null
  usage.value = res.data?.usage ?? null
  polledTermination.value = res.data?.termination_reason ?? null
...
const dimensionRows = computed<DimensionRow[]>(() => {
  ...
  if (config.max_duration_seconds) {
    const consumedSeconds =
      consumed?.duration_ms === null || consumed?.duration_ms === undefined
        ? null
        : consumed.duration_ms / 1000
    rows.push(
