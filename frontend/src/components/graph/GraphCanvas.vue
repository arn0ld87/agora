...
import { buildGraphRenderData } from './graphPanelData'
import { getLinkMidpoint, getLinkPath } from './graphPanelGeometry'
...
const renderGraph = () => {
  ...
  const { nodes, edges, getColor } = buildGraphRenderData(props.graphData, props.entityTypes)
...
onMounted(() => {
  ...
  // Erstes Render falls bereits Daten vorliegen
  nextTick(renderGraph)
})
