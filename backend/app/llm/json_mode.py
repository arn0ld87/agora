"""JSON-Mode-Orchestrierung — aus ``llm_client.py`` extrahiert (#582).

Strict-``json_schema`` → ``json_object`` → Freitext-Fallback-Bausteine:
Schema-Aufbereitung fuer OpenAI-strict und Ollamas natives ``format``-Feld,
Env-Flag-Auswertung, Codefence-/Prosa-Stripping und Best-Effort-Reparatur
abgeschnittener JSON-Antworten. Die Orchestrierung selbst (``chat_json``)
lebt weiter in :class:`app.llm.client.LLMClient`.
"""
...
def _flatten_pydantic_schema_for_ollama(model_cls: type[BaseModel]) -> Dict[str, Any]:
    ...
    flattened: Dict[str, Any] = _resolve(raw)
...
def _enforce_openai_strict_schema(model_or_schema: Any) -> Dict[str, Any]:
    ...
    enforced: Dict[str, Any] = _walk(raw)
    return enforced
