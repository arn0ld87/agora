    ...
    try:
        ...
        if max_rounds is not None:
            try:
                ...
                if max_rounds <= 0:
                    return jsonify({"success": False, "error": "max_rounds Must be positive integer"}), 400
            except (ValueError, TypeError):
                return jsonify({"success": False, "error": "max_rounds Must be valid integer"}), 400
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                    ...
                    if run_state and run_state.runner_status.value == 'running':
                        if force:
                            ...
                            try:
                                ...
                                logger.warning(f"Warning when stopping simulation: {str(exc)}")
                        else:
                            return jsonify({
                                "success": False,
                ...
                logger.info(
                    f"Simulation {simulation_id} preparation complete, resetting status to ready (previous status: {state.status.value})"
                )
        ...
        logger.error(f"Failed to start simulation: {str(exc)}")
        return jsonify({
            "success": False,
            "error": str(exc),
...
def stop_simulation():
    ...
    try:
        ...
        logger.error(f"Failed to stop simulation: {str(exc)}")
        return jsonify({
            "success": False,
            "error": str(exc),
            "traceback": traceback.format_exc() if Config.DEBUG else None,
...
def get_simulation_console_log(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to read simulation console log: {exc}")
        return jsonify({
            "success": False,
            "error": str(exc),
            "traceback": traceback.format_exc() if Config.DEBUG else None,
...
def get_run_status(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to get running status: {str(exc)}")
        return jsonify({
            "success": False,
            "error": str(exc),
            "traceback": traceback.format_exc() if Config.DEBUG else None,
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        twitter_actions = SimulationRunner.get_all_actions(simulation_id=simulation_id, platform="twitter") if not platform_filter or platform_filter == "twitter" else []
        reddit_actions = SimulationRunner.get_all_actions(simulation_id=simulation_id, platform="reddit") if not platform_filter or platform_filter == "reddit" else []
        ...
        logger.error(f"Failed to get detailed status: {str(exc)}")
        return jsonify({
            "success": False,
            "error": str(exc),
            "traceback": traceback.format_exc() if Config.DEBUG else None,
...
def get_simulation_actions(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to get action history: {str(exc)}")
        return jsonify({
            "success": False,
            "error": str(exc),
            "traceback": traceback.format_exc() if Config.DEBUG else None,
...
def get_simulation_timeline(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to get timeline: {str(exc)}")
        return jsonify({
            "success": False,
            "error": str(exc),
            "traceback": traceback.format_exc() if Config.DEBUG else None,
...
def get_agent_stats(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to get agent statistics: {str(exc)}")
        return jsonify({
            "success": False,
            "error": str(exc),
            "traceback": traceback.format_exc() if Config.DEBUG else None,
...
def get_env_status():
    ...
    try:
        ...
        message = "Environment running, ready to receive interview requests" if env_alive else "Environment not running or closed"
        return jsonify({
            "success": True,
            "data": {
                "simulation_id": simulation_id,
        ...
        logger.error(f"Failed to get environment status: {str(exc)}")
        return jsonify({
            "success": False,
            "error": str(exc),
            "traceback": traceback.format_exc() if Config.DEBUG else None,
...
def close_simulation_env():
    ...
    try:
        ...
        logger.error(f"Failed to close environment: {str(exc)}")
        return jsonify({
            "success": False,
            "error": str(exc),
            "traceback": traceback.format_exc() if Config.DEBUG else None,
