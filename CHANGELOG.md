...
- **Issue #17 Plan-Dokument** `docu/issue-17-rpc-redis-plan.md`. Konkreter Phasen-Plan für die RPC/Interview-IPC-Migration auf Redis Pub/Sub: was Subprocess-seitig (`backend/scripts/run_*_simulation.py`) angepasst werden muss, Tests-Strategie, Smoke-Test-Anforderungen, Backout-Plan, Aufwand-Schätzung. Vollmigration ist mehrkomponentig + braucht Live-Test gegen Docker-Stack — bewusst nicht autonom umgesetzt.
- **Regression-Tests** `tests/test_event_bus_redis_rpc_delegation.py` (3) sichern die Phase-B-File-Delegation für RPC-Channels: `RedisEventBus.publish`/`subscribe`/`request_response` müssen die File-Bus-Pfade nutzen, nicht Redis. Werden Issue #17 absichtlich rotwerfen, sobald die Migration startet.
