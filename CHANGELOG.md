...
### Fixed (der DEBUG-Modus lieferte die Exception-Message an den Client aus — 2026-08-04)

- **`_debug_extra` in [`api_responses.py`](backend/app/utils/api_responses.py) schrieb bei `Config.DEBUG=true` die vollständige `str(exc)` als `debug_error` in den 500-Response.** Eine Exception-Message trägt in der Praxis genau das, was nicht über die Leitung soll: Dateipfade, Verbindungsparameter, im Fehlertext mitgeschleifte Schlüssel. In Produktion (`DEBUG=false`) gab es die Lücke nie — das Feld entsteht dort gar nicht erst —, in Dev und Staging dagegen bei jedem ungefangenen Fehler. Ausgeliefert wird jetzt nur noch `debug_error_class` mit Modul und Qualname der Exception (`builtins.OSError`). Das volle Detail bleibt im Log, wo `_logger.exception` es ohnehin samt Traceback ablegt.
- **Die Lücke war im Testbestand bereits beschrieben, nur nicht scharf geschaltet.** `test_graph_ontology_upload_atomicity.py` prüft mit einem Sentinel im Fehlertext, dass weder Secret noch Pfad im Response-Body landen — unter `FLASK_DEBUG=true` reichte `_debug_extra` den Sentinel wörtlich durch. Der Testlauf ohne die Variable, also der in CI, kam nie an die Stelle. `test_debug_extra_redacts_exception_message` prüft die Funktion jetzt direkt und unabhängig von der Flask-Konfiguration.
- **Die Envelope-Assertions derselben Datei sind debug-fest gemacht, ohne weicher zu werden.** Sie verglichen den Body zeichengleich mit einem Drei-Schlüssel-Dict und wären an jedem Debug-Zusatzfeld gescheitert — auch am redigierten. Geprüft wird jetzt, dass `success`, `error` und `code` exakt stimmen und dass `debug_error_class` der einzige zulässige Zusatzschlüssel ist; jeder andere gilt weiterhin als Leck.
- **Nicht angefasst:** der separate `include_traceback`-Parameter von `json_error`. Tracebacks im Response-Body sind ein eigenes Problem mit eigener Abwägung und gehören nicht in denselben Slice.
