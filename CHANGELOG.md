...
- **Dasselbe Gate lief bisher nicht vor den Embedding-Requests** — die Discovery-Probe-Adapter (`_OpenAICompatibleAdapter`, `_OllamaAdapter`), der Ollama-Modell-Download (`embedding_ollama_pull.pull_model`) und der Legacy-Storage-Pfad (`EmbeddingService._request_headers`) sendeten ihren Bearer-Header unabhängig von `ensure_credentialed_transport_security`. Alle vier Aufrufstellen laufen jetzt vor dem jeweiligen Request durch dasselbe Gate; die Policy und `transport_security.py` selbst sind unverändert. (#1110)
