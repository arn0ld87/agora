...
- **Der Hinweis erreicht den Nutzer.** Zod-Spiegel in [`reportContract.ts`](frontend/src/contracts/reportContract.ts), Consumer in [`useReportExports.ts`](frontend/src/composables/useReportExports.ts): der Download läuft weiter, aber das Log in Schritt 4 benennt, dass die Datei keine Evidence enthält und warum.
- **Die Regressionstests fahren den HTTP-Endpunkt, nicht die Migrationsfunktion.** Das war der Kern des Defekts: `migrate_legacy_claims_to_anchored` hatte durchgehend grüne Unit-Tests, während der Export sie nicht aufrief. [`test_report_export_evidence_parity.py`](backend/tests/api/test_report_export_evidence_parity.py) exportiert über die Route und vergleicht das Ergebnis zusätzlich mit dem Lese-Pfad — wird eine der beiden Stellen künftig einseitig geändert, geht dieser Vergleich rot. Der Frontend-Test nutzt als Fixture die wörtliche Antwort des echten `ReportExportService`, nicht ein handgebautes Objekt.
