...
`npm run lint:backend` läuft jetzt **default-strict** auf `app/ tests/` (Excludes nur in `pyproject.toml`: `.venv`, `uploads`, `log`, `logs`). Der frühere gescopte Whitelist-Rollout ist abgeschlossen.
