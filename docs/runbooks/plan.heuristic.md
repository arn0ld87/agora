...
**Status:** offizielle Heuristik-Datei. Konsolidiert aus: Backup-Master-Mapping (Stand 2026-05-03), Audit-ADR-Snapshot vom 2026-05-04 ([`docs/archive/history/2026-05-04-plan-heuristic-adr-snapshot.md`](../archive/history/2026-05-04-plan-heuristic-adr-snapshot.md)), Code-Verifikation vom 2026-05-04.
...
Operative Findings: [`PLAN.md`](../../PLAN.md). Test-Counts und Layer-Status: [`docs/status.md`](../status.md).

---

## 1. Architektur-Entscheidungen (ADRs in Kurzform)

Volltext-ADRs gehören nach `docs/adr/`. Hier die geltenden Kernentscheidungen.
...
| **M10.3** | ✅ | Dependency Risk Register erweitern | `agora-doc-worker` | `/agora-next-task` | Haiku | Eskalationspfad-Sektion + neue Upstream-Release-Watch-Spalte in `docs/dependency-risk-register.md` (Owner-Spalte war bereits vorhanden). |
| **M10.4** | ✅ | Auth-Zielbild-ADR | `agora-doc-worker` (+ Lead) | `/agora-next-task` | Haiku + Senior | `docs/adr/0001-auth-model.md` **Accepted 2026-05-04** (Option A: Single-User-only-v1). `/api/status` liefert jetzt `auth_mode: "single_user_token"` (Code-Update in `_get_auth_mode()`). Folge: README/security-Update bleibt offen. |
...
| **M13.3** | ⬜ | Third-Party License-Report | `agora-doc-worker` | `/agora-next-task` | Haiku | `docs/THIRD-PARTY-LICENSES.md` CI-gepflegt |
| **M13.4** | ⬜ | F13 Doku-Konsolidierung (`docs/archive/history/`) | `agora-doc-worker` | `/agora-next-task` | Haiku | `find docs/ -maxdepth 1 -type f \| wc -l` ≤ 25 |
...
| TD-13 | Doku-Fragmentierung | niedrig | 130+ Files in `docs/` ohne klare Vorderbühne. | M13.4 |
...
- [ ] `docs/status.md` „Aktive Slices" zeigt M9.6/M10.1/M10.4 statt F1/F2/F3.
- [ ] `docs/runbooks/plan.heuristic.md` existiert.
- [ ] User-Drop-Files unter `docs/archive/history/2026-05-04-*.md` archiviert.
