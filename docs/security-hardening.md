...
### Supabase-JWT und Multi-User (ADR-0018, #1613)

- **Prüfung nach Plan §15:**
  - Signatur, `iss`, `aud`, `exp`/`nbf` mit 30 s Toleranz, `iat` und `sub` als UUID.
...
- **Row Level Security (#1615):** `FORCE ROW LEVEL SECURITY` auf Projekten, Simulationen, Runs, Reports, Workspaces und Mitgliedschaften. Die App setzt je Transaktion `agora.workspace_id` oder den System-Kontext, eine Verbindung ohne Kontext sieht nichts. Im Tenant-Modus bricht der Start ab, wenn die Laufzeitrolle Superuser, `BYPASSRLS` oder Owner ist. Die Supabase-Rolle `authenticated` darf nur lesen, gefiltert über `auth.uid()`. Die Testmatrix aus §18 läuft roh gegen die Tabellen unter einer eingeschränkten Rolle.
...
| `AGORA_CODEX_HOME` | nur fuer `codex_cli` | — (Pflicht im Override) | Dediziertes Codex-Credential-Verzeichnis. Nur wirksam mit `deploy/compose/docker-compose.codex-cli.yml`; der Standard-Stack mountet keine CLI-Credentials. Niemals auf `~/.codex` zeigen lassen. |
