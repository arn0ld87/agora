...
**Bezug:** [`PLAN.md § Status-Sync 2026-05-04`](../../PLAN.md#status-sync-2026-05-04), [`docs/runbooks/plan.heuristic.md § ADR-0001 Local-first`](../runbooks/plan.heuristic.md), Issue [#106](https://github.com/arn0ld87/agora/issues/106)
...
- README + `docs/security.md` ergänzen explizite Warnung: „Public-Internet-Deployment nicht supported. Für Mehrbenutzer-Betrieb ist v2 in Planung."
- `/api/status` liefert `auth_mode: "single_user_token"`.
- Token-Rotation: dokumentiert als Container-Neubau-Prozedur in `docs/security.md`.
...
1. **Local-first ist das Kernprinzip** ([ADR-0001 in `plan.heuristic.md`](../runbooks/plan.heuristic.md)). LLM-/Embedding-/Graph-Workloads sind teuer, langsam, schwer multi-tenant-sicher. Lokale Ollama-/Neo4j-Setups passen zum ursprünglichen Use Case.
...
- **README + `docs/security.md`** bekommen einen klaren Block: „Agora v1.0 ist Single-User-Only. Public-Internet-Deployment ist nicht supported. Für Mehrbenutzer-Betrieb v2 abwarten oder eigenes Auth-Frontend zwischen Proxy und Backend setzen."
...
- **Token-Rotation-Prozedur** wird in `docs/security.md` dokumentiert: Stop-Container, neuer `AGORA_AUTH_TOKEN` in `.env`, Container-Rebuild, Frontend setzt neuen Token zur Laufzeit. Kein File-System-State-Verlust.
...
1. **Doku-Slice** (Haiku, ~1 Tag):
   - README + `docs/security.md` Single-User-Block ergänzen.
