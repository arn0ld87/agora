...
Vier Referenzadapter in `app/services/decisions/`: `RuleProvider` (deterministisch, keine LLM-Kosten), `FakeProvider` (Tests), `LLMProvider` (ruft ausschließlich `LLMClient.chat_json`, ADR-0017 — kein zweiter roher LLM-Pfad; `cost_micros` bleibt `0`, weil `chat_json` selbst ins Run-Budget-Ledger bucht, ADR-0016), `JevDecisionProvider` (mappt auf Jevs eigene fragen-batch-förmige API, prüft die Antwort auf Vollständigkeit je Fragen-ID und berechnet `cost_micros` über die bestehende `PricingRegistry` — ein `"jev"`-Preiseintrag in `model_pricing.json` mit USD 0,042/Mio. Input-Tokens, Output kostenlos).
