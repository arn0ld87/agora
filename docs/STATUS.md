...
- **Der Reportstatus bildet den Zustand des Laufs ab** (dito): `run_degradations` am Report führt gescheiterte Simulation, unvollständige Runden, angeforderte Interviews ohne Ergebnis, fehlgeschlagene Abschnitte, erzwungene Endgenerierung und fehlende Abschnitts-Metadaten. Abgestuft wird nur bei `severity="blocking"` — ein Bericht über eine noch laufende Simulation weist seinen Zwischenstand aus, nennt sich aber nicht unvollständig. Der Simulationsstatus wurde im Snapshot bislang gelesen und weggeworfen; `get_report()` liest die Mängel jetzt auch wieder zurück, sonst meldete jede API-Antwort einen ungestörten Lauf. Deterministische Red-Team-Invarianten (`assert_run_invariants`) prüfen genau die Konstellationen, die das LLM-Red-Team des Referenzlaufs übersah — sie laufen auch, wenn der LLM-Aufruf entfällt
