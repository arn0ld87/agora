...
### Projektmetadaten: Vertrag und Port, Ablage unverändert
...
`AGORA_PROJECT_BACKEND` schaltet die Ablage getrennt von `AGORA_METADATA_BACKEND` und `AGORA_LLM_PROFILE_BACKEND` und steht im Default auf `file`. Der Wert `postgres` ist als Konfiguration gültig, hat aber **noch keinen Adapter**: `Config.validate()` lehnt ihn beim Start mit einer Begründung ab, statt ihn beim ersten Projektzugriff in einen Importfehler laufen zu lassen. Die Tabelle `agora.projects` existiert nicht — weder als Migration noch als Modell.

Zwei Festlegungen, die beim späteren Schema gelten: `project_id` behält sein Format `proj_<12 Hexstellen>` und wird **keine** `uuid`, weil es der Verzeichnisname unter `uploads/projects/` ist; und `workspace_id` kommt nicht vor, weil Multi-User eine eigene, freizugebende Phase ist.
