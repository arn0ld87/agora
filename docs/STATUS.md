...
- Claim-Extraktion: Datei- und Evidence-Pfad sehen denselben bereinigten Abschnittstext ([#1316](https://github.com/arn0ld87/agora/issues/1316)). `process_section` reinigt einmal über `_clean_section_content` und reicht das Ergebnis an `save_section` wie an `_save_evidence_section` weiter; vorher bekam nur der Dateipfad den gereinigten Text und `<simulated_quote>`-Rohmarkup wurde zu Claim-Kandidaten. `is_claim_candidate` verwirft zusätzlich Zitatblock- und Tag-Zeilen über `text_verification.is_markup_or_quote_line` (eine Definition für beide Validatoren) sowie Rest-Markup des Section-Prompts; `is_atomic_claim` verwirft Gliederungsansagen, die nichts behaupten und deshalb zwangsläufig als unbelegte Hypothese enden. Der Diskursfilter ist bewusst eng: Satzanfang oder die Kopplung aus Folgend-Verweis und passivem Darstellungsverb
