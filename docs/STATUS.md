...
Für **Graph-Build** ist die zweite Grenze aus Slice 1.1 seit Slice 1.3 (#1472b) geschlossen: `GraphBuilderService.add_text_batches` schreibt nach jedem committeten Chunk einen `GraphBuildCheckpoint` (Menge bereits abgeschlossener Chunk-Indizes, nicht nur ein Höchstwert-Cursor — die Chunks laufen parallel und schließen außerhalb ihrer Ursprungsreihenfolge ab) atomar ins Projektverzeichnis. `POST /api/runs/<id>/resume` verarbeitet bei einem passenden Checkpoint (gleicher Graph, gleiche Chunk-Größe/-Overlap/-Methode) nur noch die fehlenden Chunks, statt neu zu beginnen; `resume_capability` zeigt das jetzt auch für den SIGTERM- und den Startup-Reconciliation-Pfad korrekt an, nicht nur für einen In-Process-Fehlschlag. Passt der Checkpoint nicht mehr (z. B. geänderte Chunk-Parameter), fällt die Route sauber auf einen vollen Restart zurück. **Prepare- und Report-Jobs** haben weiterhin **keinen** persistierten Resume-Zustand — das bleibt in #1472 offen (Slice 1.4/#1472c für Prepare) und ein 0.10-Release-Thema.
...
Das Modell allein umzustellen wäre weiterhin riskant, wenn die hier beschriebenen Schutzmechanismen umgangen würden: bei gleicher Dimension landen Vektoren zweier Modelle im selben Index (genau die Korruption aus #1417), bei abweichender Dimension gehen inkompatible Query-Vektoren an den Altindex — der Dimensionswächter (#263) fängt nur letzteres, und auch das nur am Index, nicht am Modell. Der harte Riegel oben und der Cutover-Schutz aus Slice 2.2 verhindern beides; die Oberfläche kann das Modell weiterhin **nicht** wechseln, sagt das aber laut, statt es vorzutäuschen.

Offen bleibt: die `VECTOR_DIM`-SSoT (Slice 2.3) und eine Legacy-View für Bestandsgraphen (Slice 2.4). Ein Bestandssystem, bei dem eine Migration bereits vor Slice 2.2 mit dem alten Verhalten gestartet wurde, trägt diesen Zwischenzustand unverändert weiter — Slice 2.2 repariert das nicht rückwirkend.

**Bekannte Lücke (Frontend-Drift):** Der Zod-Spiegel `frontend/src/contracts/embeddingContract.ts` (`EmbeddingIndexStatusSchema`) kennt den neuen `building`-Status noch nicht. Dieser Slice wurde als Backend-Refactor-Worker ohne Frontend-Zugriff ausgeführt; das Nachziehen ist als Folgearbeit offen, bevor eine UI den `EmbeddingIndexVersion`-Status anzeigt.
