...
- `POST /api/runs/<id>/replay` übernimmt `platform`/`max_rounds`/`enable_graph_memory_update`/Graph-Memory-ID jetzt 1:1 aus dem Original-Manifest statt stillschweigend `platform="parallel"` und Runner-Defaults zu setzen; übersteuerbar bleibt nur die Modell-Route. Ein Replay-Run bekommt sein eigenes Draft-Manifest mit `deviations` (dokumentierte Abweichungen vom Original, heute nur die Route). Ein Alt-Manifest ohne diese Parameter kann nicht ehrlich 1:1 repliziert werden und liefert `409` statt eines stillen Defaults.
