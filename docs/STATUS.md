...
- **Operative Zahlen werden an ihren Beleg gebunden, statt ihre Herkunft zu behaupten** ([#1359](https://github.com/arn0ld87/agora/issues/1359) A): Schwellen trugen Herkunft und Beleglage so, wie das Modell sie mitlieferte — im Referenzlauf standen vier Wochen aus dem Seed-Dokument als `model_proposal` mit leerer `evidence_refs`-Liste. Gebunden wird jetzt über Wert plus Einheit im Belegtext (Zahlwörter bis zwölf eingeschlossen, Einheitenwort dicht hinter der Zahl). Ein `seed_corpus`-Beleg macht die Zahl zur `document_requirement`, ausschließlich agentengestützte Belege zum `simulation_proposal`; Web- und Graph-Belege bestätigen sie, ohne die Herkunft umzubenennen. Fehlt jeder Beleg, fällt eine als verbindlich ausgewiesene Zahl auf `model_proposal`/`heuristic` zurück — die Schwellen-Entsprechung zu `verified_needs_an_evidence_ref`
