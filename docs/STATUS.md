...
- Interview-Auszählung: `report_agent/interview_consensus.py::build_consensus_item` erzeugt je Sektion ein Item vom Typ `agent_interview_consensus` mit expliziter Prozentangabe, damit Regel 2 Mengenaussagen über Stakeholder überhaupt prüfen kann ([#1357](https://github.com/arn0ld87/agora/issues/1357)). Es trägt `source_kind=agent_action`, nicht `agent_quote`: eine Auszählung ist keine Stakeholder-Stimme und darf `cross_stakeholder_for_high` (ADR-0002 Anker 4) nicht im Alleingang erfüllen. `inferred` scheidet aus, weil Anker 5 jedes solche Item aus `high`-Claims verwirft und das Aggregat einen sonst belegbaren Claim herunterstufen würde. Gezählt wird über `persona_role_family`; die Zahl der Familien steht als `cluster_count` im Item und wird ausgewiesen, nicht verrechnet. Unter drei Stimmen oder ohne Stimmungswerte entsteht kein Item
