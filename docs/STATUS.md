...
- **Der Reportstatus bildet den Zustand des Laufs ab** (dito): `run_degradations` am Report führt gescheiterte Simulation, unvollständige Runden, angeforderte Interviews ohne Ergebnis und fehlgeschlagene Abschnitte; jeder Eintrag stuft `completed` auf `incomplete` ab. Der Simulationsstatus wurde im Snapshot bislang gelesen und weggeworfen. Deterministische Red-Team-Invarianten (`assert_run_invariants`) prüfen genau die Konstellationen, die das LLM-Red-Team des Referenzlaufs übersah — sie laufen auch, wenn der LLM-Aufruf entfällt
