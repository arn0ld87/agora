...
**[#1417](https://github.com/arn0ld87/agora/issues/1417) ist damit nicht geschlossen.** Seit Slice 2.1 lösen Lese- und Schreibpfad Index- und Property-Namen zwar über den Store auf (`resolve_active_entity_index()` / `resolve_active_fact_index()`), statt sie als Literale zu tragen — die Namen sind also nicht länger verdrahtet. Umgeschaltet wird damit aber noch nichts: ohne aktive Index-Version liefern beide Methoden exakt die Legacy-Namen zurück, und den Statuswechsel auf eine neue Version nimmt erst der Cutover in Slice 2.2 vor. Eine abgeschlossene Migration schaltet den Betrieb bis dahin nicht um.
...
2. #1417 — **Teil erledigt.** Der Laufzeitpfad folgt der aktiven Store-Konfiguration statt ausschließlich der `.env`, eine Konfiguration, die nicht zum Inhalt des aktiven Index passt, wirft, und seit Slice 2.1 lösen Reads und Writes ihre Index- und Property-Namen über den Store auf statt über Literale. **Offen bleibt der Cutover selbst** (Slice 2.2): solange keine Index-Version aktiv gesetzt wird, liefert die Auflösung die Legacy-Namen, und ein Modellwechsel über die Oberfläche ist weiterhin nicht möglich — siehe Abschnitt „Embeddings".
