...
- Ohne expliziten `ai_model_ref`-Override seedet der Endpoint die Modell-Route seit #1686 aus der im Original-Manifest erfassten Route statt aus den aktuellen Workspace-Defaults; ist die Original-Connection nicht mehr auflösbar, liefert er `409 manifest_route_unresolvable` statt still auf Defaults zurückzufallen (Frontend-Mapping dieses Codes in `RunReplayDialog.vue` steht noch aus). Die aufgelöste Replay-Route wird jetzt immer — nicht nur bei explizitem Override — mit der Original-Route verglichen; Abweichungen landen in `deviations`. Das Draft-Manifest des Replay-Runs wird vor dem Subprozess-Start geschrieben (vorher: danach, mit dem Risiko eines dauerhaft im Status `draft` hängenden Runs bei sofortigem Prozessende) und trägt `inputs.simulation_config_hash` der tatsächlich verwendeten Branch-Konfiguration statt des 1:1 kopierten Original-Hashs.
