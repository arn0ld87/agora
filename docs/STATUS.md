...
- 2026-07-13: Onboarding Slice 5.6 final — Playwright-E2E für `AiModelPicker` mit fünf echten Tests statt Skeleton-`.skip`: Tastatur-Navigation, Suche, verfügbare Online-Option, Abwesenheit des Offline-Modells und Run-Snapshot. Der E2E-Compose-Override enthält den `mock-models-nginx`-Service; `global-setup` erzeugt dedizierte Test-Provider-Connections. `LlmRoutingTestId` ist SSoT für `stageRow`/`stageSave` und `llm-routing-run-id`; der Chat-Mode filtert nur explizite `unsupported_capabilities`. `scripts/e2e-up.sh` ergänzt `AGORA_SECRET_KEY` für den Secrets-Store. PR [#707](https://github.com/arn0ld87/agora/pull/707); echte Playwright- und Pre-Push-Gate-Läufe auf dem armserver bleiben bewusst offen.
