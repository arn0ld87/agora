...
3. **Entitäts-/Persona-Kohärenz** — Alias/Koreferenz und Domänendrift (#1470/#1471). Weiterhin offen.
...
7. **Operations-Nachweis** — Fresh Install, Backup/Restore, Upgrade/Rollback, Release-Artefakte (#766). **Teil erledigt:** Der Restore-Drill ist ausführbar und maschinell prüfbar (`restore-drill.sh`, `restore_verify.py`, Zielschutz gegen Overwrite im eigenen Checkout, `migration_baseline.py` für Vorher/Nachher-Vergleiche, #1512/#1514). **Offen:** der tatsächlich durchgeführte Fresh-Host-Restore-/Upgrade-/Rollback-Smoke mit echtem Backup — das Werkzeug ist kein Nachweis.
