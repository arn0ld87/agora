...
Der neue Replay-Run bekommt sein eigenes Draft-Manifest (`replayed_from_run_id` zeigt auf das Original) mit einem `deviations`-Feld: einer Liste von `{field, original, replay}`-Einträgen für jede tatsächliche Abweichung vom Original — heute ausschließlich die Modell-Route, da sie die einzige überschreibbare Größe ist. Ohne Override ist `deviations` leer.
