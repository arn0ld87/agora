...
- [`deployment-prod.md`](deployment-prod.md) — Hardening-Pflicht.
...
[`security.md`](security.md), Slice 5 + Followup
...
| Trigger | Aktion |
|---|---|
| Neue Version | `git pull` + `docker compose -f docker-compose.yml -f docker-compose.prod.yml build agora` + `up -d --no-deps --force-recreate agora`. Siehe [`deployment-prod.md`](deployment-prod.md). |
...
- Sicherheits-Phasenchronik: [`security.md`](security.md).
