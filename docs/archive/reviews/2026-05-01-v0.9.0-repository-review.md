...
| F1 / Slice 7: Deployment-Doku Dev + Prod-Like | ✅ erledigt | [`e472301`](../../../commit/e472301) — `docs(deployment): add deployment-dev.md + deployment-prod.md` |
| F2 / Slice 8: Security-Threat-Model | ✅ erledigt | [`77e0cfe`](../../../commit/77e0cfe) — `docs(security): add security-threat-model.md` |
| F3 / Slice 9: Operations + Backup/Restore | ✅ erledigt | [`6251747`](../../../commit/6251747) — `docs(ops): add operations.md + backup-restore.md` |
| F4 / Slice 10: Release-Process-Doku | ✅ erledigt | [`bde34f4`](../../../commit/bde34f4) — `docs(release): add release-process.md` |
| F4-Folge / Slice 11: Versions-Sync auf 0.9.0 | ✅ erledigt | [`53bd2db`](../../../commit/53bd2db) — `fix(release): sync backend version sources to 0.9.0` |
| F5 / Slice 12: Test-Coverage SSRF + Upload + Cypher + Auth-Mode | ✅ erledigt | [`00e8492`](../../../commit/00e8492) — `test(security): add ssrf/upload/cypher/auth-mode regression tests` |
| F6 / Slice 13: Branch-Cleanup + Plan-Abschluss | ✅ erledigt | dieser Slice |
...
| `AGORA_ALLOW_ANONYMOUS=true` in Prod-Healthcheck sichtbar | ✅ (Slice 12) | `test_anonymous_in_healthcheck.py` (7 Cases) + `_get_auth_mode()` in `api/status.py` |
...
| Upload-Grenzen: Endung, PDF-Header, 50 MB, Path-Traversal | ✅ (Slice 12) | `test_upload_limits.py` (10 Cases) |
| SSRF-Blocker: `localhost`, `127.0.0.1`, `169.254.169.254`, IPv6 `::1` | ✅ (Slice 12) | `test_ssrf_blocker.py` (10 Cases) |
| Cypher-Label-Sanitizer gegen Backticks / ungültige Entity-Typen | ✅ (Slice 12) | `test_cypher_label_sanitizer.py` (27 Cases) |
| Report-/Simulation-Logs ohne Tokens | ✅ | `test_logger_redaction.py` |

**Doku-Cross-Check (Review-Forderung vs. Bestand in `docs/`):**

| Geforderte Doku | Status |
|---|---|
| `docs/auth.md` — Token-Header, Ticket-Flow, Query-Token-Deprecation | ✅ (Slice 5) |
| `docs/dependency-risk-register.md` — ignorierte CVEs, Owner, Frist | ✅ (Slice 4) |
| `docs/security-threat-model.md` — Assets, Trust Boundaries, Restrisiken | ✅ (Slice 8 / F2) |
| `docs/deployment-dev.md` — lokaler Dev-Betrieb Vite + Flask | ✅ (Slice 7 / F1) |
| `docs/deployment-prod.md` — Gunicorn, Reverse Proxy, Tailscale | ✅ (Slice 7 / F1) |
| `docs/backup-restore.md` — Neo4j, Uploads, Reports, Simulationen | ✅ (Slice 9 / F3) |
| `docs/operations.md` — Logs, Healthchecks, Ausfälle | ✅ (Slice 9 / F3) |
| `docs/release-process.md` — Versionsquellen, Changelog, Tags | ✅ (Slice 10 / F4) |
