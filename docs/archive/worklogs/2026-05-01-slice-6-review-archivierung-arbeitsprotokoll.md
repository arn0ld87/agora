...
- `claude/v0.9.0-frontend-version` ist 1 Doku-Commit vor `origin/main`:
  ...
  - Inhalt: `agora_repository_review.md` (Review) + `docs/archive/old-plans/rolle-du-bist-temporal-otter.md` (Rollenbeschreibung, gehört nicht ins Repo).
...
2. Neue archivierte Fassung als `docs/2026-05-01-v0.9.0-repository-review.md` angelegt:
   ...
   - **Doku-Cross-Check-Tabelle**: Review-Forderung vs. Bestand in `docs/` (8 Zeilen, 2×✅, 6×❌).
   - **Original-Inhalt unverändert** unterhalb des Statusblocks erhalten.
3. Folge-Plan als `docs/2026-05-01-v0.9.0-review-folge-slices-plan.md` angelegt:
...
- `docs/archive/old-plans/rolle-du-bist-temporal-otter.md` aus dem Quell-Branch — Rollen-/Persona-Beschreibungen gehören nicht ins Projekt-Repo.
- Die Datei `agora_repository_review.md` im Repo-Root — Doku-Artefakte gehören nach `docs/`, nicht in den Root.
...
- Manuelle Cross-Checks der Status-/Doku-/Test-Tabellen gegen `git log origin/main` und `ls docs/` / `ls backend/tests/`.
