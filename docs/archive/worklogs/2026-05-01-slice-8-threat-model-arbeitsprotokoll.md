...
**Bezug:** [`docu/2026-05-01-v0.9.0-review-folge-slices-plan.md`](2026-05-01-v0.9.0-review-folge-slices-plan.md), Sub-Slice F2.

## Ziel

Das implizite Threat-Model aus `security-hardening.md` und `auth.md` in
...
- F2-Scope laut Plan:
  - Eine neue Datei `docu/security-threat-model.md`.
...
- Akzeptanzkriterium: Doku konsistent mit `auth.md`,
  `dependency-risk-register.md`, `security-hardening.md`; `npm run check`
  gruen.
- Bestand:
  - `auth.md` deckt Token-Header-Vertrag, Ticket-Flow,
    Frontend-Storage-Optionen.
  - `security-hardening.md` listet Phase 1/2/3 + P1 + Slice 3
    chronologisch.
...
2. `docu/security-threat-model.md` strukturiert:
   ...
   - **Mitigation-Mapping-Tabelle**: zehn Threats → Slice/Phase →
     ...
     „Phasen-Chronologie in `security-hardening.md`“ und
...
3. Verweise auf bestehende Dokumente am Kopf und in den jeweiligen
   Mitigation-Bloecken (auth.md, security-hardening.md,
   dependency-risk-register.md, deployment-prod-like.md). Kein
...
| Datei | Aktion | LOC ca. |
|---|---|---|
| `docu/security-threat-model.md` | neu | 240 |
| `CHANGELOG.md` | edit (`[Unreleased]` → neuer Slice-8-Block oben unter `### Docs`) | +2 |
| `docu/2026-05-01-slice-8-threat-model-arbeitsprotokoll.md` | neu | dieses File |
...
- Mitigation-Mapping verlinkt nur Dateien/Slices, die tatsaechlich im
  Repo existieren (Cross-Check via `git ls-files docu/` fuer
  Doku-Eintraege, `find backend/app -name '...'` fuer Code-Pfade).

## Akzeptanzkriterien (laut Plan)

- [x] `docu/security-threat-model.md` existiert.
- [x] Doku konsistent mit `auth.md`, `dependency-risk-register.md`,
      `security-hardening.md` (Verweise + Mitigation-Mapping).
