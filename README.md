...
A SQLAlchemy/Alembic-backed PostgreSQL layer exists alongside the file/JSON/SQLite stores (self-hosted Supabase is an optional Compose overlay). Repository adapters cover **project, simulation, job, report, and LLM profile metadata**; the optional workspace path has a PostgreSQL-only adapter. Each migrated metadata store keeps its file/SQLite default and has its own `AGORA_*_BACKEND` setting. Provider secrets remain in the file-backed Fernet store, and project documents, simulation artifacts, and report contents remain files. The shipped default does not require PostgreSQL; the adapter and activation details are in [`docs/STATUS.md`](./docs/STATUS.md).
...
`install.sh` prepares the local `.env` from [`.env.example`](./.env.example); [`.env.docker.example`](./.env.docker.example) documents the container settings. Set credentials locally and keep `.env` out of Git.

| Setting | Purpose |
|---|---|
| `SECRET_KEY`, `AGORA_AUTH_TOKEN`, `AGORA_SECRET_KEY`, `AGORA_FERNET_KEY` | Application signing, master access, and encrypted stores; generated by `install.sh`. |
| `NEO4J_PASSWORD`, `NEO4J_URI`, `REDIS_URL` | Graph and live-state connections; adjust them for services outside the default Compose network. |
| `LLM_API_KEY`, `LLM_BASE_URL`, `LLM_MODEL_NAME` | Initial HTTP-provider settings; provider routes can also be configured in the application. |
...
From the repository root, `bun run backend` starts only Flask and `bun run frontend` starts only Vite. The scripts below match the checked-in package configuration:
...
Run `bun run build` for the frontend production build. The [pre-push gate](./docs/runbooks/pre-push-gate.md) documents the scope-specific contract, schema, lint, typecheck, and test checks used before a PR push. PostgreSQL schema migrations, when that optional backend is enabled, run from `backend/` with `uv run alembic -c migrations/alembic.ini upgrade head`.

## Repository layout

| Path | Contents |
|---|---|
| `backend/` | Flask API, Pydantic contracts, services, migrations, and Python tests. |
| `frontend/` | Vue application, Zod contract mirrors, and frontend tests. |
| `schemas/` | Generated contract schemas shared with the frontend. |
| `docs/` | Current architecture, operations, API references, decisions, and historical research. |
| `scripts/`, `deploy/`, `supabase/` | Quality/development scripts and optional deployment infrastructure. |
...
Prompt injection from untrusted model observations/source material is partially hardened: the single-platform tool loop wraps and neutralizes it (#1224), the parallel simulation path and fallback rounds using native CAMEL `LLMAction()` do not.
