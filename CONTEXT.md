...
7. **Exklusiver Prepare-Schreiber.** Ein zweites `POST /api/simulation/prepare` wird bei Zustand `preparing` vor Run-, Task- und Artefakterzeugung mit HTTP 409 und `simulation_prepare_in_progress` abgelehnt. `force_regenerate` umgeht diesen Guard nicht (#1271).
...
| `fixed-code` | parallele Prepare-Aufrufe schrieben dieselben Simulationsartefakte | Seit #1271 wird der zweite Aufruf im Zustand `preparing` vor Run- und Task-Erzeugung mit HTTP 409 abgelehnt. |
